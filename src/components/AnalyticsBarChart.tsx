        ...
        <div className="space-y-2 pt-2 border-t border-border/40">
          ...
          {perHabitData.map(({ habit, completedDays, totalDays, pct }) => (
            <div key={habit.id} className="flex items-center gap-3">
               <span className="flex h-5 w-5 items-center justify-center text-sm shrink-0">{habit.icon ? <HabitIcon name={habit.icon} size={14} /> : <Target className="h-3.5 w-3.5 text-text-muted" />}</span>
