...
const ACHIEVEMENTS: Achievement[] = [
  { id: 'first-step', name: 'First Steps', description: 'Complete your first check-in', icon: 'Footprints', xpReward: 50, condition: async (ctx) => ctx.totalCheckIns >= 1 },
  { id: 'week-warrior', name: 'Week Warrior', description: '7-day streak', icon: 'Zap', xpReward: 100, condition: async (ctx) => ctx.streak >= 7 },
  { id: 'two-weeks', name: 'Consistent', description: '14-day streak', icon: 'Flame', xpReward: 150, condition: async (ctx) => ctx.streak >= 14 },
  { id: 'centurion', name: 'Centurion', description: '100-day streak', icon: 'Shield', xpReward: 500, condition: async (ctx) => ctx.streak >= 100 },
  { id: 'year-master', name: 'Year Master', description: '365-day streak', icon: 'Trophy', xpReward: 2000, condition: async (ctx) => ctx.streak >= 365 },
  { id: 'habit-builder', name: 'Habit Builder', description: 'Create 3 habits', icon: 'ClipboardList', xpReward: 75, condition: async (ctx) => ctx.habitCount >= 3 },
  { id: 'habit-master', name: 'Habit Master', description: 'Create 5 habits', icon: 'Target', xpReward: 150, condition: async (ctx) => ctx.habitCount >= 5 },
  { id: 'productive', name: 'Productive', description: 'Complete 10 check-ins total', icon: 'CheckCircle', xpReward: 100, condition: async (ctx) => ctx.totalCheckIns >= 10 },
  { id: 'dedicated', name: 'Dedicated', description: 'Complete 50 check-ins total', icon: 'Dumbbell', xpReward: 250, condition: async (ctx) => ctx.totalCheckIns >= 50 },
  { id: 'committed', name: 'Committed', description: 'Complete 100 check-ins total', icon: 'Heart', xpReward: 400, condition: async (ctx) => ctx.totalCheckIns >= 100 },
  { id: 'legend', name: 'Legend', description: 'Complete 500 check-ins total', icon: 'Star', xpReward: 1000, condition: async (ctx) => ctx.totalCheckIns >= 500 },
  { id: 'momentum-master', name: 'Momentum Master', description: 'Complete 12 of last 14 days', icon: 'TrendingUp', xpReward: 200, condition: async (ctx) => ctx.momentum.completed >= 12 },
  { id: 'perfect-week', name: 'Perfect Week', description: 'All habits hit their weekly targets for the current week', icon: 'Sparkles', xpReward: 300, condition: async (ctx) => ctx.consecutiveWeeksAtTarget >= 1 },
  { id: 'early-bird', name: 'Early Bird', description: 'Check in before 8 AM', icon: 'Sunrise', xpReward: 50, condition: async (ctx) => ctx.checkInHour < 8 },
  { id: 'night-owl', name: 'Night Owl', description: 'Check in after 10 PM', icon: 'Moon', xpReward: 50, condition: async (ctx) => ctx.checkInHour >= 22 },
  { id: 'streak-freezer', name: 'Streak Freezer', description: 'Use a streak freeze', icon: 'Snowflake', xpReward: 25, condition: async (ctx) => ctx.freezesUsed > 0 },
  { id: 'marathon', name: 'Marathon', description: '30-day streak', icon: 'Footprints', xpReward: 300, condition: async (ctx) => ctx.streak >= 30 },
  { id: 'half-year', name: 'Half Year Hero', description: '180-day streak', icon: 'Award', xpReward: 1000, condition: async (ctx) => ctx.streak >= 180 },
  { id: 'on-fire', name: 'On Fire', description: '5-day streak', icon: 'Flame', xpReward: 75, condition: async (ctx) => ctx.streak >= 5 },
  { id: 'target-hacker', name: 'Target Hacker', description: 'Hit weekly target for the first time', icon: 'Target', xpReward: 100, condition: async (ctx) => ctx.weeklyTargetHit >= 1 },
  { id: 'consistent-crusher', name: 'Consistent Crusher', description: 'Hit all weekly targets for 4 consecutive weeks', icon: 'Repeat', xpReward: 500, condition: async (ctx) => ctx.consecutiveWeeksAtTarget >= 4 },
  { id: 'data-collector', name: 'Data Collector', description: 'Log 100 numeric entries', icon: 'BarChart3', xpReward: 150, condition: async (ctx) => ctx.numericEntries >= 100 },
  { id: 'data-master', name: 'Data Master', description: 'Log 500 numeric entries', icon: 'Brain', xpReward: 400, condition: async (ctx) => ctx.numericEntries >= 500 },
  { id: 'multi-tracker', name: 'Multi-Tracker', description: 'Track habits in 3+ categories', icon: 'Layers', xpReward: 100, condition: async (ctx) => ctx.categoryCount >= 3 },
  { id: 'well-rounded', name: 'Well-Rounded', description: 'Check in all categories in one day', icon: 'Palette', xpReward: 200, condition: async (ctx) => ctx.allCategoriesCheckedToday },
  { id: 'comeback-kid', name: 'Comeback Kid', description: 'Return after 3+ day gap and reach 7-day streak', icon: 'RotateCcw', xpReward: 250, condition: async (ctx) => ctx.streak >= 7 && ctx.isComeback },
];
